...
TMP_DIR = os.path.join(REPO, "P2", "data", "duckdb_tmp_s2_test")
...
t0 = time.time()
print("1. Loading ground truth S2...")
con.execute(f"""
...
print("4. Generating train_s2_v2_raw...")
t_gen = time.time()
con.execute("""
...
""")
print(f"train_s2_v2_raw generated in {time.time() - t_gen:.2f}s")

res = con.execute("SELECT count(*), count(distinct source1_entity_id) FROM train_s2_v2_raw").fetchone()
print(f"train_s2_v2: rows={res[0]:,}, distinct_s1={res[1]:,}")

pos = con.execute("""
    SELECT count(*) 
    FROM train_s2_v2_raw c
    JOIN gt_s2 gt ON c.source1_entity_id = gt.source1_entity_id AND c.matched_entity_id = gt.matched_entity_id
""").fetchone()[0]
print(f"train_s2_v2 positives: {pos:,}")

con.execute("DROP TABLE train_s2")
con.execute("DROP TABLE train_s1")
con.execute("DROP TABLE gt_s2")

print("Aggregating S2 per S1 into Parquet...")
out_parquet = os.path.join(REPO, "P2", "data", "v2_s2_agg.parquet")
con.execute(f"""
    COPY (
        SELECT 
            c.source1_entity_id,
        ...
        LEFT JOIN (
            SELECT source1_entity_id, TRIM(UNNEST(string_split(matched_entity_ids, ','))) AS matched_entity_id
            FROM read_csv('{GT_PATH}', delim='\\t', header=true, all_varchar=true)
            WHERE matched_entity_ids IS NOT NULL AND TRIM(matched_entity_ids) <> '' AND matched_entity_ids LIKE '%S2-%'
...
if os.path.exists(TMP_DIR):
    shutil.rmtree(TMP_DIR)
print(f"S2 completed in {time.time() - t0:.2f}s")
